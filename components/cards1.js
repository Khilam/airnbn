...
function Cities(props) {
    return (
        ...
        <img className="explore-img my-1" src={props.img}/>
...
function TunisianCities() {
    return(
     <div className="container mb-5">
        <h3 className="my-3">Explorer les environs</h3>
       <div className="cities">
          {tunisianCities.map(element =>
            <div>
               <Cities name = {element.name} link = {element.link} img = {element.img} time = {element.time}/>     
            </div>
          )}
       </div>
     </div>
